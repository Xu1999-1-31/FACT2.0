...
file delete -force ../PtECO/${top_design}
set multi_scenario_working_directory ../PtECO/${top_design}
set multi_scenario_merged_error_log ../PtECO/${top_design}/error_log.txt
 ...
 foreach mode {func} {
  foreach rc_corner {rcworst rcbest cworst cbest typical} {
   ...
   create_scenario \
    ...
    -specific_data {run_design_eco.tcl} \
  }
 }
}
...
remote_execute {
 update_timing -full
 check_timing
 report_timing -nosplit -nets -input_pins -transition_time -capacitance -significant_digit 6 -max_path 1000 -pba_mode exhaustive -nworst 5 -slack_lesser_than 10
 report_design
...
write_changes -format icc2tcl -output ../PtECO/$top_design/change.tcl
