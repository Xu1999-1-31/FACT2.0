...
def Read_PtRpt(inrpt):
    if not os.path.exists(inrpt):
        raise FileNotFoundError(f"Timing Rpt file not found: {inrpt}")
    ...
    for timing_list in timing_matches:
        ...
        for i in range(1, len(index1)):
            ...
            if(index1[i].find('(rise') != -1 or (index1[i].find('(fall') != -1 and index1[i].find('(falling') == -1) ):
    ...
    for i in range(len(startpoint)):
        paths[i].Startpoint = startpoint[i].replace(' ','').replace('\n', '')
        paths[i].Endpoint = endpoint[i].replace(' ','').replace('\n', '')
        paths[i].slack = slack[i]

    print(f"Timing Rpt at {inrpt.split('/')[-2]} Readed")
    return paths

if __name__ == "__main__":
    paths = Read_PtRpt("aes_cipher_top.rpt")
    for path in paths:
        print(path)